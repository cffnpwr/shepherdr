...
/// show a Dock icon; it is meant to run as a menu bar resident app only.
///
/// # Errors
///
...
pub fn run() -> tauri::Result<()> {
    ...
    // Nothing to do in the first instance: there is no window to raise, and the second instance
    // exits on its own without having touched a service.
    #[cfg(desktop)]
    ...
    app.run(|app_handle, event| {
        ...
        // the tray's quit item, `applicationWillTerminate` on a Cmd+Q or a logout, and the last
        // window closing all converge here, right before the event loop tears down. Blocking the
        // main thread on the stop sequence is what holds the process open until every service has
        // actually finished.
